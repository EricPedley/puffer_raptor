...
class Quadcopter3DGates(VecEnv):
    def __init__(
        ...
        self.reset_on_ground = reset_on_ground
    ...
    def step_wait(self):
        ...
        if not self.reset_on_ground:
            if pos_new[:, 2] > 0:
                pos_new[:, 2] = 0  # prevent going through ground. negative z is up
                new_states[:, 3:6] = 0
                new_states[:, 6:8] = 0
                # clamp new_states[:,8] (z velocity) to be negative so takeoff is possible
                if new_states[:, 8] > 0:
                    new_states[:, 8] = 0
                new_states[:, 9:12] = 0
        ...
        for gate_type_idx, gate_indices in enumerate(gates_to_check):
            ...
            R_world_to_gate_current = np.array(
                [
                    ...
                    [
                        np.zeros_like(cos_yaw_current),
                        np.zeros_like(cos_yaw_current),
                        np.ones_like(cos_yaw_current),
                    ],
                ]
            ).transpose(2, 0, 1)  # Shape: (N, 3, 3)

            # Transform positions to gate frame
            pos_old_gate_frame_current = np.einsum(
                "nij,nj->ni", R_world_to_gate_current, pos_old_relative_current
            )
            pos_new_gate_frame_current = np.einsum(
                "nij,nj->ni", R_world_to_gate_current, pos_new_relative_current
            )

            # Check if drone passed through gate plane
            passed_gate_plane_current = (pos_old_gate_frame_current[:, 0] < 0) & (
                pos_new_gate_frame_current[:, 0] >= 0
            )
            passed_gate_plane_rev_current = (pos_old_gate_frame_current[:, 0] > 0) & (
                pos_new_gate_frame_current[:, 0] <= 0
            )

            # Check if drone is within gate boundaries
            within_gate_hole_current = (
                np.abs(pos_new_gate_frame_current[:, 1]) < gate_hole / 2
            ) & (np.abs(pos_new_gate_frame_current[:, 2]) < gate_hole / 2)
            within_gate_size_current = (
                np.abs(pos_new_gate_frame_current[:, 1]) < gate_outside / 2
            ) & (np.abs(pos_new_gate_frame_current[:, 2]) < gate_outside / 2)
            ...
            gate_collision_current = (
                passed_gate_plane_current & ~within_gate_hole_current
            ) | (passed_gate_plane_rev_current & within_gate_size_current)
        ...
        out_of_bounds = np.any(
            np.abs(new_states[:, 0:2]) > 50, axis=1
        )  # edges of the grid
